...
Note that when don't want to split items (all child items are grouped together), you can use the [alignment properties][common-types] on `Vertical`/`Horizontal` to achieve the same effect (no `StretchSpacer` in the below example):
