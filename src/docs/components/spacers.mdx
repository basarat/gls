...
We've covered issues with using margin (specifically their collapsing nature which makes them hard to manage and maintain reliably) in our [spacing-principle][principle-spacing]. Another issue is that `marginTop` can easily get lost in code reviews. 
...
Here is a button pushed to the bottom of its parent `Vertical` by a preceding `FlexSpacer`: 
...
And here is a button pushed to the right of its parent `Horizontal` by a preceding `FlexSpacer`: 
