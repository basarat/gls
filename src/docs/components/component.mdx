...
## component
There is a `gls.component` function that takes any set (subset or fullset) of `ComponentProps` and returns props with any `ComponentProps` omitted + processsed to place into the `className` prop (taking into account any existing `className` as well), that you can use to power to root of your component.
