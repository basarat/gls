...
## component
There is a `gls.component` function that takes any set (subset or fullset) of `ComponentProps` and returns props with any `ComponentProps` omitted + processsed and placed into the `className` prop (taking into account any existing `className` as well). You can then use this `className` in your component.

We look at examples below.

## Creating a component
You can use any partial set of `ComponentProps` in your component e.g here as just use the `SizeProps`: 
...
Any props tht are not a part of `ComponentProps` are yours to handle and returned as is from `gls.component` e.g. below you can see the `error` prop passing through the component function without any processing: 
