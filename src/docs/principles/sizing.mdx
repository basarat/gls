...
In the example below, we have an explicit sized `Vertical` (background `lightskyblue`) with an inner flex-sized `Vertical` (background `darkorange`). The inner vertical keeps its size to match that of the outer container (observed below as you don't see the background of the parent-lightskyblue as it is filled in by the child-darkorange).
...
`sizing=flex` takes precedence on any explicit sizing e.g. `width` has no effect in the following example:

<Playground>
  <Horizontal height={100} spacing={0} style={{backgroundColor: "lightskyblue"}}>
    {/* Explicit width 100 */ }
    <Box sizing='flex' width={100} style={{backgroundColor: "limegreen"}}>Width 100px will not work because of `sizing=flex`</Box>
  </Horizontal>
</Playground>
