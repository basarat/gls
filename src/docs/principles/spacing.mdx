...
- Fundamentally external e.g. outside the width of the component in the recommended `border-box` box sizing (width of the component only includes the content,padding and border):
...
### Space among children: the `spacing` prop
...
Because the input was margin free, it can easily be reused in any given context. We can easily compose it with a`label` to create a `label+input` set:
...
> Component design tip: `padding` can be used in any component that has an explict border (e.g buttons, user profile cards).
...
So a page level form layout component would have padding, whereas a reusable form component would not. You can observe this pattern in the example below where the two `Vertical`s for the reuseable `First/Middle/Last` name component do not have a padding (enabling seemless reuse): 
