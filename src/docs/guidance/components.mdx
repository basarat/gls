...
> If a component is going to occupy a significant portion of user's interaction time, then it should be `sizing:'flex'` by default. 
...
As an example consider an input with `sizing:'flex'` as default [using the `gls.component` function][component]:
