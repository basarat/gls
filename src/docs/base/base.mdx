...
## Normalize 
Unifies the [user agent style sheets][user-agent-stylesheet] (the browser default styles), so that they are consistent. This allows you to test your code in one browser and be fairly confident that it will look the same in the other browsers ❤️
...
We take a root selector and set it up to fill the entire `body`. It is common to have a `root` element in the *index.html* of React applications e.g.
