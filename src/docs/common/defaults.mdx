...
In the pseudo code example below, you can see `Vertical` using default `verticalSpacing` (powered by the parent provider) for its `spacing`, or the explicit value when its passed in:
