...
```
Gives you direct access to the underlying react tag for any customization you might want e.g. generate a `Vertical` as a `section` tag (instead of the default `div` tag):

<Playground>
  <Vertical 
    tag={{name:'section', props: {onClick:() => alert('Nice you meet you ❤️')}}} 
...
* `TagProps`: [Allows tag control](#tagprops).
